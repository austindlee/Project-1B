...
<body>
	...
	$rowR = mysql_fetch_row($resultR);
	 if (count($rowR[0]) == 0)
		...
		echo "Average Score of $rowR[0]/5 based on $rowR[1] people. \n";
	mysql_free_result($resultR);
	$queryR = "SELECT name, time, rating, comment FROM Review WHERE mid=" . $mid . " ORDER BY time DESC";
	if(!$resultR = mysql_query($queryR))
		die("Error executing query: " . mysql_error());
	echo "<br />";
	echo "<a href=\"./add_review.php?mid=$mid\">Give your input too!</a><br/>\n";
	while ($rowR = mysql_fetch_row($resultR))
	{
		echo "<br/><br/>\n";
		echo "$rowR[0] rated this movie a $rowR[2] at $rowR[1]. <br/> Comment: <br/>\n $rowR[3]\n";
	}
